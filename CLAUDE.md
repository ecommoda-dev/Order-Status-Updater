...
Build watch paths : index.js + wrangler.toml (مضيّقة 06-09-2026) — راجع "مسائل مفتوحة"
```
...
القراءة بعد النقل — **06-09-2026** (من 31-08 18:00Z لحد 06-09):

```
update : +471 صف   (آخر صف 05-09-2026 15:18Z)
login  :   +9 صف   (آخر صف 06-09-2026 04:16Z)
```

✅ **الأداة شغّالة بعد النقل** — بتكتب في D1 عادي، يعني شوبيفاي والأسرار
والـ bindings كلهم تمام.

### 🔬 إثبات إن النسخة المنشورة مصدرها الريبو (مش رفع يدوي)

> «الأداة شغّالة» **مش** إثبات إن البناء نجح — بتخدم من آخر نسخة منشورة حتى لو
> البناء وقع. الإثبات القاطع هنا اتاخد 06-09-2026 من `workers_get_worker_code`:

| الدليل | قبل النقل (رفع يدوي) | بعد النقل (Workers Builds) |
|---|---|---|
| اسم الجزء في الرد | `name="worker.js"` | **`name="index.js"`** ← مطابق لـ `main` في `wrangler.toml` |
| الحجم | 92,010 حرف · 1,975 سطر | 63,967 حرف · 1,415 سطر |
| التعليقات | كاملة (بلوك الرأس العربي) | **متشالة** — esbuild |
| بادئة الحزم | مفيش | `__defProp` / `__name` / `// index.js` |

والثوابت كلها سليمة في المنشور: `TOOL_NAME = "order_status"` ·
`VERSION = "4.3.0"` · `ecommoda-dev.github.io` · `targetLabel` · `reasonSource`.

⚠️ **ونتيجة جانبية مهمة:** بصمة المهارات في `index.js` **مابتوصلش للمنشور**
(esbuild بيشيل التعليقات). مصدر الحقيقة للبصمة = `CLAUDE.md` هنا، مش الكود
المنشور. متحققش البصمة بـ `workers_get_worker_code` — هترجع فاضية وتستنتج غلط.

### توقيت أول نشر من git

```
05:49:35Z  دمج PR #1 على main
05:52:50Z  commit "Update wrangler.toml" (سطر فاضي — commit تشغيل البناء)
05:53:11Z  الـ Worker اتنشر  ← بعد الـ push بـ 21 ثانية
```
...
| ecommoda-html-builder | v1.0.0 (ما قبل النظام) |
...
- **`Build watch paths` اتضيّقت لـ `index.js` + `wrangler.toml` (06-09-2026).**
  ⚠️ **أي ملف جديد يعتمد عليه الـ Worker لازم يتضاف للقايمة** (`package.json`،
  فولدر `src/`، ملف config) — وإلا الـ Worker يفضل على نسخة قديمة **من غير أي
  رسالة**، نفس شكل الفخ الخامس بالظبط.
- **🟡 تحقق §13-ب ناقص نصّه.** الاختبارين الإلزاميين:
  - ✅ **إيجابي على `wrangler.toml`** — commit لمس `wrangler.toml` بس يوم
    06-09 ونشر بعد 21 ثانية. يعني الـ chips **مش** الصيغة المكسورة
    (chip واحد باسم `index.js wrangler.toml`)، وإلا ماكانش هيبني أصلاً.
  - ❌ **إيجابي على `index.js` لسه ما اتعملش** — الخطر المتبقي إن
    `index.js` يكون **مش** متسجّل كـ chip مستقل. لو كده، كل تعديل كود
    جاي هيتفلتر في صمت والأداة تتجمّد للأبد.
  - ❌ **سلبي (تعديل md/HTML لوحده ما ينشرش) لسه ما اتعملش.**
